...
License: GPL
Release: 1
Source: http://sourceforge.net/projects/binaryeditor/
Url: http://bed.dse.nl
...
make
%install
########if test -e /usr/lib/bed-2.27.1/uninstallbed.sh; then bash /usr/lib/bed-2.27.1/uninstallbed.sh --force ; fi
make  installfiles ROOTDIR=$RPM_BUILD_ROOT
%post
ln -T -fs /usr/lib/bed-2.27.1 /usr/share/bed
cd /usr/lib/bed-2.27.1
bash ./addmagic
if  which update-menus 2>/dev/null; then update-menus; fi
%files
%config /usr/lib/bed-2.27.1
/usr/share/bed
/usr/bin/bed
