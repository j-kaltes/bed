...
int calcsizes(void) override{
   ...
   virtual int addsubpart(ScreenPart *part,int pos=-1)  override {
      ...
      if(filterpresent(this)) {
            part->calcsizes();
            Bitfield *bitf=((Bitfield *)filters[filnr]);
            bitf->addposition(pos,part->getbytes(),nrsubparts);
            bitf->isSigned[pos]=part->isSigned();
           }
...
int Editor::changebitfield(int kind) {
   ...
   char ans[100];
   if(has_super(cur,HasSub)) {
        HasSub *hassub=(HasSub *)cur;
        active=hassub->activesubpart();
      //  ScreenPart *part=hassub->subpart(active);
     //   bool prev= bit->isSigned[active];
      // bit->isSigned[active]=part->isSigned();
     //  message("%s isSigned=%d prev=%d",part->getname(),part->isSigned(),prev);
        }
