...
//Format: want:fbits:number:b;by,b;by,
    ...
    Bitfield::Bitfield(Bitfield const &rev): Filter(rev),maxlen(0),number(rev.number),filebits(rev.filebits) ,
        ...
        {
        part=NULL;
        memcpy(isSigned,rev.isSigned,sizeof(bool)*number);
...
*/
    int Bitfield::extendtolen(int newnumber) {
        if(newnumber<=number)
            return number;
        bool *isSignedtmp=new bool[newnumber];
        Bitnr *bittmp=new Bitnr[newnumber];
        Bitnr *bytetmp=new Bytenr[newnumber];
        if(number>0) {
            memcpy(isSignedtmp,isSigned,number*sizeof(bool));
    ...
    int Bitfield::rmpos(int pos) { 
        ...
        number--;
        if(number>pos) {
            memmove(isSigned+pos,isSigned+pos+1,(number-pos)*sizeof(bool));
            memmove(bitnr+pos,bitnr+pos+1,(number-pos)*sizeof(Bitnr));
            memmove(bytenr+pos,bytenr+pos+1,(number-pos)*sizeof(Bytenr));
            }
        calcsizes();
        return pos;
        }
...
//        memset(output,'\0',bytesscreen);
        for(int i=0;i<number;i++) {
            const int nrbytes=bytenr[i];
            const int nrbit=bitnr[i];
            const int bij=startbit+nrbit;
            const int bijbytes=bij/8;
            const int bijbits=bij%8;
            if(nrbytes) {
                const int bij1=bij-1; 
                const int bijbytes1=bij1/8;
                const int bijbits1=bij1%8;
                if(isSigned[i]&&(inptr[bijbytes1]&(1<<bijbits1))) {
                    memset(outptr,(uint8_t)0xFF,nrbytes);
                    }
                 else {
                    memset(outptr,(uint8_t)0,nrbytes);
                    }
                bitcpy(outptr,inptr,0,startbit,nrbit);
                outptr+=nrbytes;
                }
            inptr+=bijbytes;
            startbit=bijbits;
